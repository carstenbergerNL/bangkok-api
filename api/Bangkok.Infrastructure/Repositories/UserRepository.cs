...
public class UserRepository : IUserRepository
{
    ...
    public async Task<Guid> CreateAsync(User user, CancellationToken cancellationToken = default)
    {
        ...
        using (connection)
        {
            connection.Open();
            const string sql = @"
            INSERT INTO dbo.[User] (Id, Email, DisplayName, PasswordHash, PasswordSalt, IsActive, CreatedAtUtc, IsDeleted)
            VALUES (@Id, @Email, @DisplayName, @PasswordHash, @PasswordSalt, @IsActive, @CreatedAtUtc, 0)";
    ...
    public async Task UpdateAsync(User user, CancellationToken cancellationToken = default)
    {
        ...
        using (connection)
        {
            ...
            const string sql = @"
            UPDATE dbo.[User]
            SET Email = @Email, DisplayName = @DisplayName, PasswordHash = @PasswordHash, PasswordSalt = @PasswordSalt,
    ...
    public async Task HardDeleteAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        using (connection)
        {
            ...
            using (var transaction = connection.BeginTransaction())
            {
                ...
                await connection.ExecuteAsync(
                    new CommandDefinition("DELETE FROM dbo.UserRole WHERE UserId = @Id", new { Id = id }, transaction, cancellationToken: cancellationToken)).ConfigureAwait(false);
                await connection.ExecuteAsync(
